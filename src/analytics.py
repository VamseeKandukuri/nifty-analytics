...
def rsi(close: pd.Series, period: int = RSI_PERIOD) -> pd.Series:
    ...
    Wilder's original method is an exponential average with alpha = 1/period,
    not the simple rolling mean many quick implementations use. The two diverge
    by a few points on trending stocks, and every charting package worth
    comparing against uses Wilder's, so this matches what you would see on a
    broker terminal.
    """
    delta = close.diff()
    ...
    def _wilder_average(changes: pd.Series) -> pd.Series:
        # Seed at bar `period` with a plain average of the first `period` changes,
        # then let the recursion take over. Handing the raw series straight to
        # ewm() instead would seed from a single day's change and stay several
        # points off the value any charting package reports.
        seeded = changes.astype(float).copy()
        seeded.iloc[:period] = np.nan
        seeded.iloc[period] = changes.iloc[1:period + 1].mean()
        return seeded.ewm(alpha=1 / period, adjust=False).mean()

    avg_gain = _wilder_average(gain)
    avg_loss = _wilder_average(loss)

    strength = avg_gain / avg_loss
    values = 100 - (100 / (1 + strength))

    # A stretch with no down days sends avg_loss to zero: RSI is 100 there.
    # A completely flat stretch leaves both averages at zero and RSI undefined.
    values = values.where(avg_loss != 0, 100.0)
    values = values.where(~((avg_gain == 0) & (avg_loss == 0)), np.nan)
    return values
...
def macd(
    ...
    # The slow EMA has not seen enough data to mean anything before `slow` bars.
...
def with_indicators(history: pd.DataFrame, period: str) -> pd.DataFrame:
    """
    Moving averages, RSI and MACD, computed on the full price series and only
    then sliced to the requested window.

    Order matters here. Every one of these indicators carries state from before
    the visible window: a 200-day average needs 200 prior days, and an EMA never
    fully forgets its start. Slicing first and calculating second would draw a
    blank 200 DMA on a one-month chart and an MACD that starts from a standing
    stop instead of continuing the real trend.
    """
    if history.empty:
        return history
    ...
    for window in (50, 100, 200):
        frame[f"DMA{window}"] = close.rolling(window).mean()
...
with_moving_averages = with_indicators

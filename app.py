...
period_column, rsi_column, macd_column = st.columns([4, 1, 1])
with period_column:
    period = st.radio(
        "Chart period",
...
chart_frame = analytics.with_indicators(history, period)
if chart_frame.empty:
    st.info("No price data for that window.")
else:
    st.plotly_chart(
        charts.price_chart(chart_frame, row["symbol"], period, show_rsi, show_macd),
        width="stretch",
    )
    ...
    for window in (50, 100, 200):
        ...
        if pd.notna(value):
            ...
            readings.append(f"{side} its {window} DMA ({theme.rupees(value)})")

    lines = []
    if readings:
        lines.append(f"Trading {', '.join(readings)}.")

    if show_rsi and pd.notna(latest.get("RSI")):
        rsi_value = latest["RSI"]
        zone = "above 70" if rsi_value > 70 else "below 30" if rsi_value < 30 else "between 30 and 70"
        lines.append(
            f"{analytics.RSI_PERIOD}-day RSI is {theme.number(rsi_value, 1)}, {zone}."
        )

    if show_macd and pd.notna(latest.get("MACD")) and pd.notna(latest.get("MACD_SIGNAL")):
        gap = latest["MACD"] - latest["MACD_SIGNAL"]
        lines.append(
            f"MACD ({charts.MACD_LABEL}) is {theme.number(latest['MACD'], 2)} against a "
            f"signal of {theme.number(latest['MACD_SIGNAL'], 2)}, "
            f"{'above' if gap >= 0 else 'below'} it."
        )

    if lines:
        st.markdown(f'<div class="note">{" ".join(lines)}</div>', unsafe_allow_html=True)

    st.markdown(
        '<div class="note" style="margin-top:6px">RSI uses Wilder\'s smoothing and MACD '
        "uses closing prices; both are calculated on the full price history and then "
        "cropped to the window above, so the short views are not starting from a "
        "standing stop. These describe where the price has been, not where it is going "
        "&mdash; evidence that they predict returns on their own is weak.</div>",
        unsafe_allow_html=True,
    )
